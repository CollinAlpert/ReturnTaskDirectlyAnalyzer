...
public class ReturnTaskDirectlyFixer : CodeFixProvider
{
	...
	private static SyntaxNode GetParentMethod(ExpressionSyntax expression)
	{
		...
		{
			if (parent.IsKind(SyntaxKind.MethodDeclaration) || parent.IsKind(SyntaxKind.LocalFunctionStatement) || parent.IsKind(SyntaxKind.ParenthesizedLambdaExpression))
			{
				return parent;
			}
		...
		throw new Exception("Expression is not in method.");
	}
	...
	public override ImmutableArray<string> FixableDiagnosticIds { get; } = ImmutableArray.Create(DiagnosticDescriptors.ReturnTaskDirectly.Id);
}